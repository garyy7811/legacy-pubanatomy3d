 ...
 * Time: 4:19 PM
 */
package edu.umich.med.mbni.pubanatomy.datadriven3dwindow {
...
[Bindable]
public class Dd3dViewCameraController extends LookAtController {
    ...
    [Bindable(event="panAngleChanged")]
    ...
    public function set panAngle( val:Number ):void{
        ...
        _panAngle = val;
        dispatchEvent( new Event( "panAngleChanged" ) );

        notifyUpdate();
    }

    [Bindable(event="tiltAngleChanged")]
    ...
    public function set tiltAngle( val:Number ):void{
        ...
        dispatchEvent( new Event( "tiltAngleChanged" ) );
        notifyUpdate();
    }

    [Bindable(event="distanceChanged")]
    ...
    public function set distance( val:Number ):void{
        ...
        _distance = val;
        dispatchEvent( new Event( "distanceChanged" ) );
        notifyUpdate();
    }
    ...
    public override function update( interpolate:Boolean = true ):void{
        if( _tiltAngle != _currentTiltAngle || _panAngle != _currentPanAngle ){
            ...
            if( (Math.abs( tiltAngle - _currentTiltAngle ) < 0.01) &&
                    (Math.abs( _panAngle - _currentPanAngle ) < 0.01) ){
                _currentTiltAngle = _tiltAngle;
                _currentPanAngle = _panAngle;
            }
        }
        ...
        targetObject.x = pos.x + distance * Math.sin( ( 360 - _currentPanAngle  ) * MathConsts.DEGREES_TO_RADIANS ) *
                Math.cos( ( _currentTiltAngle + 180 ) * MathConsts.DEGREES_TO_RADIANS );
        targetObject.z = pos.z + distance * Math.cos( ( 360 - _currentPanAngle ) * MathConsts.DEGREES_TO_RADIANS ) *
                Math.cos( ( _currentTiltAngle + 180 ) * MathConsts.DEGREES_TO_RADIANS );
        targetObject.y = pos.y + distance * Math.sin( ( _currentTiltAngle + 180 ) * MathConsts.DEGREES_TO_RADIANS ) * yFactor;


        if( _targetObject ){

            if( _lookAtPosition ){
                _targetObject.lookAt( _lookAtPosition, new Vector3D( 0, -1, 0 ) );
            }
            else if( _lookAtObject ){
                _targetObject.lookAt( _lookAtObject.scene ? _lookAtObject.scenePosition : _lookAtObject.position,
                                      new Vector3D( 0, -1, 0 ) );
            }
            updatedTargetPosition = new Vector3D( _targetObject.x, _targetObject.y, _targetObject.z );
        }
    }

    [Bindable]
    public var updatedTargetPosition:Vector3D;
}
}